...
#define JOB2CYCLE 50          // Job 2 execution cycle: 0.050s - Share the results: Output the joystick data to the serial console.
...
#define JOB5CYCLE 200         // Job 5 execution cycle: 0.200s - Action commands: Action any received buzzer serial commands.
...
void loop() {
  ...
  // Initialise the buzzer command action status variables.
  ...
  // Job 2 - Share the results: Output the joystick data to the serial console.
  ...
  // Job 5 - Action commands: Action any received buzzer serial commands.
  if (timeNow - timeMark5 >= JOB5CYCLE) {
    timeMark5 = timeNow;
    // Do something...
    // If we have received a buzzer serial command.
    if (pBuzzerActionD > 0) {
...
// Turn ON the buzzer at the given frequency for the duration specified.
